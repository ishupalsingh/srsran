...
TEST(srs_sib3_test, make_asn1_rrc_cell_sib3_buffer)
{
  ...
  EXPECT_TRUE(sib3_decoded.intra_freq_neigh_cell_info_present);
  EXPECT_EQ(sib3_decoded.intra_freq_neigh_cell_info.size(), sib3.intra_freq_neigh_cell_info.size());
  EXPECT_TRUE(sib3_decoded.intra_freq_black_cell_info_present);
  EXPECT_EQ(sib3_decoded.intra_freq_black_cell_info.size(), sib3.intra_freq_black_cell_info.size());
}
