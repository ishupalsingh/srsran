...
static asn1::rrc_nr::sib3_s make_asn1_rrc_cell_sib3(const sib3_info& sib3_params)
{
   ...
   for (const auto& intra_neigh_white_info : sib3_params.intra_freq_white_cell_info_r16) 
   {
    	...
    	intra_neigh_white_r16.start = intra_neigh_white_info.nr_pci_start;
    	if (intra_neigh_white_info.range.has_value()) 
	{
    	   bool rok = asn1::number_to_enum(intra_neigh_white_r16.range, intra_neigh_white_info.range.value());
    	   if (!rok) 
	      continue; // avoiding assert as of now if the number to enum conversion fails
